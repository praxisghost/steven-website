...
// ─────────────────────────────────────────────────────────────────────────────
// SUPERORGANISM GRAPH — keep the knowledge-graph data fresh on every boot.
// ─────────────────────────────────────────────────────────────────────────────
// The Superorganism page (Writing → Isms → Superorganism) renders from a static
// JSON file generated by tools/graph/build-graph.mjs. Historically that JSON
// drifted out of date because the build was only ever run by hand. Regenerating
// it here means it can never go stale: it is rebuilt from the current pages on
// every container start (i.e. on every deploy). Wrapped in try/catch so a build
// failure logs a warning and the site still serves the last-good graph JSON.
function refreshGraphData(): void {
  try {
    const script = path.join(__dirname, 'tools', 'graph', 'build-graph.mjs');
    const out = execFileSync('node', [script], { encoding: 'utf8' });
    console.log('[graph] regenerated Superorganism graph:\n' + out.trim());
  } catch (err) {
    console.warn('[graph] could not regenerate graph data (serving existing JSON):',
